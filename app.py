...
app.config['MYSQL_USER'] = 'miUser'
app.config['MYSQL_PASSWORD'] = 'xxx'  # Reemplaza con tu contraseña real
app.config['MYSQL_DB'] = 'miBd'
...
def index():
    ...
    return render_template('index.html', fecha=fecha, hora=hora, eventos=eventos, provincias=provincias)
...
def enviar_email(nombre, apellido, provincia, localidad, whatsapp, email, consulta):
    ...
    mensaje['To'] = "mi@email.com"
    mensaje['Subject'] = "Nuevo mensaje del sitio web www.lavinoteca.com.ar"
...
def manejar_formulario():
    ...
    try:
        ...
        return "<i class='fa-regular fa-circle-check'></i> Email enviado de manera correcta.", 200
    except Exception as e:
        print(f"Error al enviar email: {e}")  # Registrar el error en la consola
